...
export function ExpenseForm({ open, onClose, routeId, expense }: ExpenseFormProps) {
  ...
  const calculatedRsd = showExchangeRate && amount && exchangeRate ? amount * exchangeRate : null
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{isEditing ? t('common:actions.edit') : t('expenses.addNew')}</DialogTitle>
        </DialogHeader>
        <Form form={form} onSubmit={onSubmit} className="space-y-4">
          <FormField control={form.control} name="category" render={({ field }) => (
            <FormItem>
              <FormLabel>{t('expenses.category')}</FormLabel>
              <Select options={categoryOptions} value={field.value} onChange={field.onChange} searchable />
              <FormMessage />
            </FormItem>
          )} />
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <FormField control={form.control} name="currency" render={({ field }) => (
              <FormItem>
                ...
                <Select options={currencyOptions} value={field.value} onChange={field.onChange} />
                <FormMessage />
              </FormItem>
            )} />
          </div>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <FormField control={form.control} name="exchangeRate" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input type="number" step="0.0001" value={field.value ?? ''} onChange={(e) => field.onChange(e.target.value ? Number(e.target.value) : null)} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              <div className="flex flex-col justify-end">
                <Caption className="text-muted-foreground mb-1">{t('common:actions.inRsd')}</Caption>
                <div className="bg-muted flex h-9 items-center rounded-md px-3 text-sm font-medium">
                  {calculatedRsd != null ? formatCurrency(calculatedRsd, 'RSD') : '—'}
                </div>
              </div>
            </div>
          )}
          ...
          <FormField control={form.control} name="expenseDate" render={({ field }) => (
            <FormItem>
              ...
              <DatePicker value={field.value} onChange={(d) => field.onChange(d ?? '')} returnFormat="iso" clearable />
              <FormMessage />
            </FormItem>
          )} />
