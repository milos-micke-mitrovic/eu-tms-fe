...
export function FuelTransactionForm({ open, onClose, tankId, transactionType }: FuelTransactionFormProps) {
  ...
  const vehicleOptions = (vehiclesData?.vehicles?.content ?? []).map((v) => ({ value: String(v.id), label: v.regNumber }))
  ...
  const form = useForm<TransactionFormData>({
    ...
    defaultValues: {
      liters: 0, pricePerLiter: null, vehicleId: null, driverId: null,
      odometerKm: null, transactionDate: format(new Date(), 'yyyy-MM-dd'), notes: '',
    },
  })
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            {t('addTransaction')}
            <Badge variant={isDispense ? 'default' : 'secondary'}>{isDispense ? t('dispense') : t('refill')}</Badge>
          </DialogTitle>
        </DialogHeader>
        <Form form={form} onSubmit={onSubmit} className="space-y-4">
          ...
          {totalCost != null && (
            <div><Caption className="text-muted-foreground">Ukupno: {totalCost.toLocaleString('sr-RS', { minimumFractionDigits: 2 })} RSD</Caption></div>
          )}
          {isDispense && (
            <>
              <FormField control={form.control} name="vehicleId" render={({ field }) => (
                <FormItem><FormLabel>{t('common:fleet.vehicles.title', { defaultValue: 'Vozilo' })}</FormLabel><Select options={vehicleOptions} value={field.value ? String(field.value) : undefined} onChange={(v) => field.onChange(v ? Number(v) : null)} clearable searchable placeholder={t('common:select.placeholder')} /><FormMessage /></FormItem>
              )} />
              <FormField control={form.control} name="driverId" render={({ field }) => (
                <FormItem><FormLabel>Vozač</FormLabel><Select options={driverOptions} value={field.value ? String(field.value) : undefined} onChange={(v) => field.onChange(v ? Number(v) : null)} clearable searchable placeholder={t('common:select.placeholder')} /><FormMessage /></FormItem>
              )} />
          ...
          <FormField control={form.control} name="transactionDate" render={({ field }) => (
            <FormItem><FormLabel>Datum</FormLabel><DatePicker value={field.value} onChange={(d) => field.onChange(d ?? '')} returnFormat="iso" clearable /><FormMessage /></FormItem>
          )} />
          <FormField control={form.control} name="notes" render={({ field }) => (
            <FormItem><FormLabel>{t('common:actions.notes')}</FormLabel><FormControl><Input {...field} value={field.value ?? ''} /></FormControl><FormMessage /></FormItem>
          )} />
